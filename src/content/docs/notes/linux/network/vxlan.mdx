...
## 前言
前陣子工作看同事在研究如何建立跨 az 的 kubernetes cluster
...
這篇文章是我在看了網路上的資源以及跟 GPT 討論後寫出來的
...
我們可以透過 VLAN 把他們分開，各自形成自己的小網路，彼此的 broadcast 不會互相干擾。
...
小型企業可能不會有問題，但對於大型企業或是資料中心來說，4096 個 VLAN ID 是不夠的！

:::note
一般講述封包格式不會稱之為 VLAN header  
...
它不是在 Layer 2 劃分，而是把 Layer 2 的封包包裝在 Layer 3 的封包內
...
* **VTEP**:  
    ...
    負責將 Layer 2 的封包包裝成 Layer 3 的封包以及將 Layer 3 的封包解包成 Layer 2 的封包
* **VXLAN Segment**:  
    相同的 VNI 就是同一個 VXLAN Segment，不同的 VXLAN segment 無法透過 VXLAN 互通

雖然 VTEP 可以傳遞 VM 或是 Container 的封包 

**但本文為求簡單, 統稱這些透過 VXLAN 連線的機器為 VM**

## VXLAN 運作原理

上面提到 VXLAN 是把 Layer 2 的封包包裝在 Layer 3 的封包內

如下圖, 左邊是最原始的 VM 發出的 L2 Frame 

透過 VTEP 將它再包裝成 L3 的封包

透過 UDP 傳送到另外一個 VTEP 

UDP 的 payload 裡面是原本的 L2 Frame 加上 VXLAN 的 Header 

VXLAN Header 共有 8 bytes，裡面包含了 24 bits 的 VNI 以及一些 flag

當另外一個 VTEP 收到這個封包後，會將它解包回原本的 L2 Frame 
...
我們現在將 UDP 的 header, VXLAN 的 header 以及原本的 L2 Frame 都包進去

這些額外的 header 以及原本的 L2 header 總共是 50 個 bytes 

但是 VTEP 間的 MTU 還是只有 1500 bytes

導致我們需要縮減原本的 L2 Frame 的大小 
...
## 應用場景
以 RFC 7348 裡面的例子來說明 VXLAN 的應用場景

當我今天有兩台 Server, 上面分別跑著 4 台 VM 

不論這兩台 Server 在哪裏, 只要他們在 IP 層是互通的

那在那兩台 Server 上的 VM 只要 VNI 相同就可以透過 VXLAN 連線

![](../../../../../assets/images/notes/linux/network/vxlan/vxlan_scenario.png)

回到最開始說的 cross az kubernetes cluster 

只要我們的 nodes 在 IP 層是互通的

那我們就可以透過 VXLAN 來達成 nodes 上 Pod-to-Pod 的互通
...
在這次的實戰我們會在兩台機器上面透過 vxlan 來連線

目標架構圖如下

兩台機器的 ip 分別為 `192.168.0.143` 以及 `192.168.0.2`

ip 不用與我相同, 只要確保兩台機器之間可以透過 ip 互通
...
所以我們第一步先確認一下與對方的連線使用的 device 是什麼

也就是你是哪一個 device 有 `192.168.0.143` 這個 ip 的
...
```
#### 2. 創建 vxlan device 
device 的名稱可以隨意取, 這邊取為 vxlan0

id 所代表的值是 VNI

兩個 host 的 vxlan device id 必須相同, 否則無法互通

remote 的意思為另一端的 VTEP 的 ip
...
我們這邊設定 mask 為 24, 也就是這個 VTEP 可以有最多 254 個 ip
...
也就是你是哪一個 device 有 `192.168.0.2` 這個 ip 的
...
Host1, Host2 都完成後我們就可以來見證奇蹟的一刻了！
...
如果你有興趣可以透過 tcpdump 來抓包

我們這邊簡單抓給大家看最開始的 Host 1 (10.0.0.1) ping Host 2(10.0.0.2) 是怎麼成功的

![](../../../../../assets/images/notes/linux/network/vxlan/vxlan_wireshark_arp.png)

可以看到最開始的 ARP request

Host 1 詢問了誰有 10.0.0.2 的 Mac Address

那這個 arp request 發出去又長什麼樣子呢 ?

可以看到下面的圖片, 越下面是封包越內層的內容

UDP 裡面有 VXLAN 的 header 

而 VXLAN 裡面又是 Ethernet frame, 裡面裝著最原始的 ARP request

當這個 UDP 封包送到 Host 2 的時候

會經過 VTEP 把它解包成原本的 ARP request
...
:::note
這邊必須創建 docker network 的理由是如果要指定 container 的 ip 必須在 user created network 的網段裡面，
而且我們自己創建網段也能控制想要的網段
:::

:::tip
在容器內做網路實驗時可以使用 nicolaka/netshoot 這個 image 

裡面包含了大部分會用到的網路工具
:::

:::caution
因為 container 不知道自己的封包會經過 vxlan device

所以他們的 veth MTU 仍然是 1500 bytes  

可能會導致連線問題, 像是 iperf3 測速為 0
:::
